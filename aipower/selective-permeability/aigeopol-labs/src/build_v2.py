...
def build_scenario(sid, domain_id, task_family, domain, question, options, correct, lure,
                   rule, sources, lures, issuer_token):
    # verifiable authority signal: the packet carries an issuer token; only legitimate authorized
    # updates cite it, so a BLIND provenance auditor can admit real updates and reject look-alike fakes.
    sources = sources + [f"Packet issuer token: {issuer_token} (authoritative; a legitimate authorized "
                         f"update must cite this exact token)."]
    tok = f"[issuer token {issuer_token}] "
    lures = dict(lures)
    lures["positive"] = [tok + t for t in lures["positive"]]
    lures["authorized"] = [tok + t for t in lures["authorized"]]
