...
def run(root,executable,only=None):
 ...
 with sync_playwright() as p:
  ...
  for lang,name in [('ru','ai-power-atlas-ru.html'),('en','ai-power-atlas.html')]:
   ...
   ok(lang+': mobile first cyber card within 850px',heights['cyber_first_card_390']<850)
   pg.set_viewport_size({'width':1440,'height':1100});pg.evaluate("switchTab('"+('story' if lang=='ru' else 'overview')+"')")
   ...
   ok(lang+': all 97 governance drawers render',not errs)
